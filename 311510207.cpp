...
struct gate_info{
    ...
    double delay; //delay
    double transition;
    double toggle_time_rise ;
    double toggle_time_fall ;
    bool toggle;
    bool cs;
...
///////////////////////////////////////////////////////////////////
    ...
    gate_tmp.toggle_time_rise = 0;
    gate_tmp.toggle_time_fall = 0;
    while (getline(inFile, line)) {
    ...
    while (getline(pat, line)) {
       ...
       iii++;
        ...
        for(int i=0; i<gate_order_ptr.size();i++){  
            ...
            if(( gate_order_ptr[i]->cs ==0)&&(logic ==1))
                gate_order_ptr[i]->toggle_time_rise++;
            if(( gate_order_ptr[i]->cs ==1)&&(logic ==0))
                gate_order_ptr[i]->toggle_time_fall++;
        ...
        for (int i = 0; i < gate.size(); i++) {
            ...
            total_toggle = (gate[i].toggle_time_rise >20)? total_toggle+20:total_toggle+gate[i].toggle_time_rise;
            total_toggle = (gate[i].toggle_time_fall >20)? total_toggle+20:total_toggle+gate[i].toggle_time_fall;
